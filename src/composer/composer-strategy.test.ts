...
describe("classifyIntent — gallery detection", () => {
  it('detects gallery from "gallery" keyword', () => {
    expect(classifyIntent("show a gallery of screenshots").hasGallery).toBe(
      true,
    );
  });
  ...
  it('detects gallery from "screenshot" keyword', () => {
    expect(
      classifyIntent("include a screenshot of the dashboard").hasGallery,
    ).toBe(true);
  });

  it('detects gallery from "screenshots" keyword', () => {
    expect(classifyIntent("product screenshots showcase").hasGallery).toBe(
      true,
    );
  });
...
describe("classifyIntent — testimonials detection", () => {
  it('detects testimonials from "testimonials" keyword', () => {
    expect(
      classifyIntent("include customer testimonials section").hasTestimonials,
    ).toBe(true);
  });

  it('detects testimonials from "reviews" keyword', () => {
    expect(
      classifyIntent("show user reviews and ratings").hasTestimonials,
    ).toBe(true);
  });

  it('detects testimonials from "social proof" keyword', () => {
    expect(
      classifyIntent("add a social proof section to the page").hasTestimonials,
    ).toBe(true);
  });
});
...
describe("classifyIntent — newsletter detection", () => {
  ...
  it('detects newsletter from "subscribe" keyword', () => {
    expect(
      classifyIntent("users can subscribe for updates").hasNewsletter,
    ).toBe(true);
  });
});
...
describe("classifyIntent — pricing detection", () => {
  ...
  it('detects pricing from "billing" keyword', () => {
    expect(classifyIntent("billing and subscription options").hasPricing).toBe(
      true,
    );
  });
...
describe("classifyIntent — page type detection", () => {
  it('detects about page from "about" keyword', () => {
    expect(
      classifyIntent("create an about us page for the team").pageType,
    ).toBe("about");
  });

  it('detects about page from "team" keyword', () => {
    expect(classifyIntent("show the team and company story").pageType).toBe(
      "about",
    );
  });

  it('detects portfolio page from "portfolio" keyword', () => {
    expect(classifyIntent("build a portfolio page for my work").pageType).toBe(
      "portfolio",
    );
  });

  it('detects product page from "product" keyword', () => {
    expect(classifyIntent("create a product page for our shop").pageType).toBe(
      "product",
    );
  });

  it('detects landing page from "launch" keyword', () => {
    expect(classifyIntent("launch page for waitlist signups").pageType).toBe(
      "landing",
    );
  });
  ...
  it("defaults to saas for generic builder prompt", () => {
    expect(
      classifyIntent("create a SaaS application with features and pricing")
        .pageType,
    ).toBe("saas");
  });
...
describe("classifyIntent — brand extraction", () => {
  it('extracts brand from "for X" pattern', () => {
    expect(classifyIntent("create a landing page for Acme").brand.name).toBe(
      "Acme",
    );
  });
  ...
  it('extracts brand from "brand X" pattern', () => {
    expect(
      classifyIntent("the brand Stripe is a fintech product").brand.name,
    ).toBe("Stripe");
  });

  it("defaults brand to EMOVEL when no pattern matches", () => {
    expect(classifyIntent("just a generic SaaS page").brand.name).toBe(
      "EMOVEL",
    );
  });
});
...
describe("classifyIntent — ComposerBrief extraction", () => {
  it("returns deterministic prompt-aware brief fields", () => {
    const profile = classifyIntent(
      "Create a SaaS pricing page for ClinicFlow, a healthcare platform for clinic managers who need automated intake, with HIPAA-ready forms, SMS reminders, and analytics. Trusted by 200 clinics. Book a demo. Starts at $29/mo.",
    );

    expect(profile.brief.productName).toBe("ClinicFlow");
    expect(profile.brief.industry).toBe("healthcare");
    expect(profile.brief.audiencePhrase).toContain("clinic managers");
    expect(profile.brief.offer).toContain("automated intake");
    expect(profile.brief.differentiators).toContain("HIPAA-ready forms");
    expect(profile.brief.proofPoints).toContain("200 clinics");
    expect(profile.brief.ctaIntent).toBe("Book a demo");
    expect(profile.brief.pricingHint).toBe("$29/mo");
  });

  it("uses safe non-empty fallbacks for vague prompts", () => {
    const brief = classifyIntent("").brief;

    expect(brief.productName).toBe("EMOVEL");
    expect(brief.industry.length).toBeGreaterThan(0);
    expect(brief.audiencePhrase.length).toBeGreaterThan(0);
    expect(brief.offer.length).toBeGreaterThan(0);
    expect(brief.differentiators.length).toBeGreaterThan(0);
    expect(brief.proofPoints.length).toBeGreaterThan(0);
    expect(brief.ctaIntent.length).toBeGreaterThan(0);
    expect(brief.pricingHint.length).toBeGreaterThan(0);
  });
});
